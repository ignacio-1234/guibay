...
export function RegisterForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setIsLoading(false);
      setErrors({
        form: payload.error ?? "No pudimos crear la cuenta.",
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-8 space-y-5">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {errors.form && (
          <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700">{errors.form}</p>
        )}

        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-accent text-white font-semibold py-3 rounded-xl hover:bg-accent/90 transition-colors disabled:cursor-not-allowed disabled:opacity-60"
        >
